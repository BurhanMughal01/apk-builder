export async function handleBuild(request, env, user, ctx) {
  ...
  const profile = await getUserProfile(user ? user.id : null, env);
  if (user && (!profile || profile.builds_used >= profile.builds_limit)) {
    return json({ error: 'Quota exceeded', upgrade: true }, 403);
  }
